...
# Построение более мощной модели
model = Sequential()
...
# Сохранение модели
try:
    model.save('multi_crypto_lstm_model_powerful.keras')
    print("Модель сохранена как 'multi_crypto_lstm_model_powerful.keras'")
    if os.path.exists('multi_crypto_lstm_model_powerful.keras'):
        print(f"Файл модели успешно сохранен, размер: {os.path.getsize('multi_crypto_lstm_model_powerful.keras')} байт")
    else:
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('training_loss_powerful.png')
plt.show()
